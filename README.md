...
- **AI 评标**：支持评分项抽取、投标文件导入、多投标文件评分、批量重评、专家打分交叉审核、审计意见，以及 Markdown / Word / Excel 报告。
- **投标机会**：支持手动录入、公告文件导入、URL 导入、AI 结构化解析、机会评分、负责人/下一步动作/提醒时间、知识库匹配、Markdown 报告和 iCalendar 导出。
- **知识库与图片知识库**：文档知识库支持迁移、解析、匹配和 active task 恢复；图片知识库支持上传、分类、文件夹、标签、批量管理、正文插入、引用回溯和正文生成优先复用。
- **标书查重**：支持元数据、目录、正文重复句、完全重复图片和感知 hash 相似图片检测；支持确认/忽略/批量处理/正文忽略规则，以及带相似图片复核视图的 Markdown / Word / PDF 报告。
- **废标项检查**：支持招标/投标文件导入、废标风险/错别字/逻辑问题检查、单文件重查、忽略/删除/批量处理、证据复制，以及 Markdown / Word / PDF 报告。
- **设置、资源与开发者工具**：支持文本/生图/解析配置、主题与布局设置、资源下载离线缓存、真实 Prompt 调试、解析沙盘、导出预演和 JSON 请求实验室。

### 🚧 仍在演进

- 多项目工作区已支持在设置页查看、新建、切换、归档、恢复和删除项目，并已补充重启作用域的切换 E2E；切换后仍需重启加载对应工作区，运行时热刷新仍在增强。
- 废标项检查已包含文本型证据截图视图，标书查重已包含文本型相似图片复核视图；真实图片截屏证据、查重复杂截图裁剪/旋转/水印相似图的图片化证据仍会继续增强。
- 真实样本文档集的本地解析回归已落地；MinerU 精准 / MinerU Agent 的真实网络端到端回归仍需在配置 Token 和网络后补跑固化。

...
function registerKnowledgeBaseIpc({ knowledgeBaseService }) {
  ...
  ipcMain.handle('knowledge-base:get-active-tasks', () => knowledgeBaseService.getActiveTasks());
  ...
  ipcMain.handle('knowledge-base:import-categorized-archives', (event) => knowledgeBaseService.importCategorizedArchives(event.sender));

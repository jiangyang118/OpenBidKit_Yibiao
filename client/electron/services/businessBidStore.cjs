...
const categoryKeywords = [
  { category: 'payment', keywords: ['付款', '支付', '结算', '进度款', '验收后', '发票'] },
  { category: 'bond', keywords: ['履约保证金', '保证金', '保函', '担保'] },
  { category: 'quote', keywords: ['报价', '最高限价', '控制价', '投标报价', '分项报价'] },
  { category: 'contract', keywords: ['合同', '违约', '质保', '保修', '验收', '合同条款'] },
  { category: 'qualification', keywords: ['资格', '资质', '证书', '业绩', '证明材料', '授权书'] },
  { category: 'schedule', keywords: ['工期', '服务期', '交付期', '完成时间'] },
];
...
function inferMaterialRequirement(category, text) {
  ...
  if (category === 'quote') return '补充分项报价表、报价说明和价格有效期。';
  if (category === 'qualification') return '补充营业执照、资质证书、业绩证明、授权书等资信材料。';
...
function extractBusinessClauses(markdown, options = {}) {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (category === 'other' && !/(商务|响应|偏离|报价|合同|资信|资格|付款|保证金|服务期|工期)/.test(paragraph)) {
      continue;
    }
...
async function buildBusinessBidWordBuffer(state) {
  const source = state?.source || {};
  const clauses = Array.isArray(state?.clauses) ? state.clauses : [];
  const attachments = Array.isArray(state?.attachments) ? state.attachments : [];
  const confirmedCount = clauses.filter((clause) => clause.confirmed).length;
  ...
  const children = [
    new Paragraph({
      ...
      children: [new TextRun({ text: '商务标响应交付包', font: '宋体', size: 32, bold: true })],
    }),
    ...
    createDocxParagraph(`条款总数：${clauses.length}；已确认：${confirmedCount}；待确认：${clauses.length - confirmedCount}；高风险：${highRiskCount}；独立附件：${attachments.length}；缺失附件：${missingAttachmentCount}`),
  ];
  for (const table of getBusinessBidDeliveryTables(state)) {
    ...
    children.push(createDocxTable(table.rows));
  }
  children.push(new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({ text: '独立附件清单', font: '宋体', size: 26, bold: true })] }));
  children.push(createDocxAttachmentTable(attachments));
  children.push(new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({ text: '处理建议', font: '宋体', size: 26, bold: true })] }));
  children.push(createDocxParagraph(highRiskCount > 0 ? '存在高风险商务条款，提交前需要逐条复核并确认响应口径。' : '暂未发现高风险商务条款。'));
  children.push(createDocxParagraph(confirmedCount < clauses.length ? '仍有待确认条款，建议在导出正式商务标前完成确认。' : '所有识别条款均已确认。'));
  children.push(createDocxParagraph(missingAttachmentCount > 0 ? '存在缺失附件，请在正式导出前补齐或标记处理口径。' : '独立附件清单暂无缺失项。'));
...
function createBusinessBidStore({ db, technicalPlanStore, fileService, aiService, app, workspaceRoot }) {
  ...
  async function exportOfficePackage(options = {}) {
    ...
    if (!filePath) {
      const result = await dialog.showSaveDialog({
        title: format === 'xlsx' ? '导出商务标 Excel 表格' : '导出商务标 Word 交付包',
        defaultPath: `商务标响应交付包-${new Date().toISOString().slice(0, 10)}.${format}`,
        filters: format === 'xlsx'
    ...
    const buffer = format === 'xlsx'
      ...
      : await buildBusinessBidWordBuffer(state);
    fs.writeFileSync(filePath, buffer);
    return {
      success: true,
      message: format === 'xlsx' ? '商务标 Excel 表格已导出' : '商务标 Word 交付包已导出',
      filePath,
      bytes: buffer.length,
      format,
    };

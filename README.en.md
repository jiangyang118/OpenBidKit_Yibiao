...
- **AI evaluation**: Extract scoring items, import bid documents, score multiple bid files, batch re-score, cross-check expert scores, generate audit opinions, and export Markdown / Word / Excel reports plus committee meeting-minutes templates.
- **Tender opportunities**: Supports manual input, announcement file import, URL import, AI structured parsing, opportunity scoring, owner / next action / reminder fields, multi-round follow-ups, announcement / communication attachments, knowledge-base matching, Markdown reports, and iCalendar export.
...
- **Duplicate checking**: Covers metadata, outline, repeated sentences, exact image matches, and perceptual-hash similar images; similar-image evidence now covers compression, scaling, rotation, bottom-right watermark/badge hints, and 16x16 fine-grained local crop boxes; supports confirm / ignore / batch handling / reusable text ignore rules, plus Markdown / Word / PDF reports with a similar-image review view.
- **Rejection-risk checks**: Supports tender/bid document import, rejection-risk / typo / logic checks, single-file recheck, ignore / delete / batch handling, evidence copying, and Markdown / Word / PDF reports with evidence indexes, text-based evidence screenshot views, local-PDF page screenshot candidates, and generated evidence crop images.
...
- Multi-project workspaces can now be viewed, created, switched, archived, restored, removed, duplicated, exported, and imported from Settings. Switching the active project hot-reloads the current session's workspace services and is covered by E2E.
- Rejection reports now include text-based evidence screenshot views, page screenshot candidate matching, generated evidence crop images, and page screenshot candidates for local PDF, DOCX, DOC, WPS, and images returned by MinerU. Duplicate-check reports include similar-image review views, visual evidence metadata, and graphical PDF review cards.
- Real sample-document regression for local parsing is in place, and scanned PDFs / images can now use local OCR with page screenshot candidates. Local OCR prefers PaddleOCR and falls back to Tesseract when the shared runtime is unavailable. MinerU Agent live-network parsing regression passes. MinerU Accurate remains an optional enhancement path; in an environment with token and network access, run `YIBIAO_RUN_MINERU_E2E=1 YIBIAO_MINERU_TOKEN=<token> npm run test:unit -- fileServiceParserRegression.test.ts` to record the final zip image asset and page screenshot candidate result.
